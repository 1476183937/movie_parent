(function() {
	...
	var a = function(q) {
		...
		n._init = function() {
			...
			x += "</p>";
			var y = b();
			$("body").append(x + v + y + '</div><div class="adp-wraper-backshadow" style="z-index: ' + (t.index - 1) +
				'"></div>');
			if (t.isAsync) {
				o = f(null);
		...
		n._initFirstLevelData = function() {
			for (var w = 0; w < t.level; w++) {
				...
				$("#adp-wraper-" + k).children("div").eq(w).children("ul").append('<span class="adp-empty-text">' + t.emptyText +
					"</span>")
			}
			if (o.length > 0) {
			...
			$("#adp-wraper-" + k + " div ul").delegate("li", "click", function() {
				...
				if (z < G) {
					for (var F = z; F < t.level; F++) {
						$("#adp-wraper-" + k).children("div.ul-div").eq(F).children("ul").empty().append(
							'<span class="adp-empty-text">' + t.emptyText + "</span>");
						if (t.isShowBtn) {
							s(F)
						}
					}
					...
					if (x != undefined) {
							...
							if (t.isAsync) {
								...
								if (y.name == $(this).text() && y.code == $(this).data("code")) {
									if (y.children != null && y.children.length > 0) {
										...
										$("#adp-wraper-" + k).children("div.ul-div").eq(z).children("ul").append('<span class="adp-empty-text">' +
											t.emptyText + "</span>")
									}
									break
								}
						...
						$("#adp-wraper-" + k).children("div.ul-div").eq(z).children("ul").append('<span class="adp-empty-text">' + t
							.emptyText + "</span>")
					}
		...
		n._clearSelectedData = function() {
			...
			for (var w = 1; w < t.level; w++) {
				$("#adp-wraper-" + k).children("div.ul-div").eq(w).children("ul").empty().append('<span class="adp-empty-text">' +
					t.emptyText + "</span>");
				if (t.isShowBtn) {
					s(w)
				}
		...
		n._setSelectedData = function(E) {
			...
			if (typeof(C) == "number") {
				...
				for (var y = C + 1; y < t.level; y++) {
					$("#adp-wraper-" + k).children("div.ul-div").eq(y).children("ul").empty().append(
						'<span class="adp-empty-text">' + t.emptyText + "</span>");
					if (t.isShowBtn) {
						s(y)
					}
