...
jQuery("#bar-chart-12").length && ((chart12 = new ApexCharts(document.querySelector("#bar-chart-12"), barOptions12)).render(),
...
if (jQuery("#amChartWorldMap").length) {
	am4core.useTheme(am4themes_animated), (chart = am4core.create("amChartWorldMap", am4maps.MapChart)).geodata =
		am4geodata_worldLow, chart.projection = new am4maps.projections.Miller;
	var worldSeries = chart.series.push(new am4maps.MapPolygonSeries);
	worldSeries.exclude = ["AQ"], worldSeries.useGeodata = !0;
	var polygonTemplate = worldSeries.mapPolygons.template;
	polygonTemplate.tooltipText = "{name}", polygonTemplate.fill = chart.colors.getIndex(0), polygonTemplate.nonScalingStroke = !
		0, (hs = polygonTemplate.states.create("hover")).properties.fill = am4core.color("#367B25");
	var usaSeries = chart.series.push(new am4maps.MapPolygonSeries);
	usaSeries.geodata = am4geodata_usaLow;
	var hs, usPolygonTemplate = usaSeries.mapPolygons.template;
	usPolygonTemplate.tooltipText = "{name}", usPolygonTemplate.fill = chart.colors.getIndex(1), usPolygonTemplate.nonScalingStroke = !
		0, (hs = usPolygonTemplate.states.create("hover")).properties.fill = am4core.color("#367B25")
}
if (jQuery("#chartdiv").length && jQuery(document).ready(function() {
		am4core.ready(function() {
			...
			var e = {
					...
					BA: {
						...
						continent_code: "EU",
						continent: "Europe",
						maps: ["bosniaHerzegovinaLow", "bosniaHerzegovinaHigh", "bosniaHerzegovinaCantonsLow",
					...
					DO: {
						...
						continent_code: "NA",
						continent: "North America",
						maps: ["dominicanRepublicLow", "dominicanRepublicHigh", "dominicanRepublicMuniLow",
					...
					US: {
						...
						continent_code: "NA",
						continent: "North America",
						maps: ["usaLow", "usaHigh", "usaTerritoriesLow", "usaTerritoriesHigh", "usaTerritories2Low",
							"usaTerritories2High"
						]
					},
					UY: {
			...
			o.tooltipText = "{name}", o.nonScalingStroke = !0, o.strokeOpacity = .5, o.fill = am4core.color("#eee"), o.propertyFields
				.fill = "color", o.states.create("hover").properties.fill = a.colors.getIndex(9);
			...
			i.tooltipText = "{name}", i.nonScalingStroke = !0, i.strokeOpacity = .5, i.fill = am4core.color("#eee"), i.states
				.create("hover").properties.fill = a.colors.getIndex(9), o.events.on("hit", function(e) {
					e.target.series.chart.zoomToMapObject(e.target);
					var t = e.target.dataItem.dataContext.map;
					t && (e.target.isHover = !1, r.geodataSource.url = "https://www.amcharts.com/lib/4/geodata/json/" + t +
						".json", r.geodataSource.load())
				});
			var c = [];
			for (var s in e)
			...
			d.events.on("hit", function() {
				...
				}), d.icon = new am4core.Sprite, d.padding(7, 5, 7, 5), d.width = 30, d.icon.path =
				"M16,8 L14,8 L14,16 L10,16 L10,10 L6,10 L6,16 L2,16 L2,8 L0,8 L8,0 L16,8 Z M16,8", d.marginBottom = 10, d.parent =
				a.zoomControl, d.insertBefore(a.zoomControl.plusButton)
		})
	}), 
	...
	options = {
		...
		series: [{
			name: "Desktops",
			data: [0, 41, 35, 51, 49, 62, 69, 91, 148,444,223,10, 41, 35, 51, 49, 62, 69, 91, 148,444,223,150,156,120]
		},
		],
		dataLabels: {
			enabled: !1
	...
	// (chart = new ApexCharts(document.querySelector("#apex-basic_2"), options)).render()
}

// jQuery("#apex-basic_2").length) if (jQuery("#apex-basic_2").length)
if (jQuery("#apex-basic_2").length) {
	options = {
		chart: {
			height: 350,
			type: "line",
...
if (jQuery("#user-visit").length) {
	...
	// (chart = new ApexCharts(document.querySelector("#apex-basic"), options)).render()
	(chart = new ApexCharts(document.querySelector("#user-visit"), options)).render()
}
//每小时用户访问量统计 结束


// 自定义部分：top10的类别 开始
if (jQuery("#top-category-area").length) {
	options = {
		chart: {
			height: 350,
			type: "area"
		...
		series: [{
			name: "点击数",
			data: [31, 40, 28, 51, 42, 109, 700,28, 51, 42]
		}, {
			name: "收藏数",
			data: [11, 32, 45, 32, 34, 52, 41, 32, 45, 32]
		}, {
			name: "评分数",
			data: [25, 52, 55, 58, 34, 62, 79,55, 48, 34]
		}, {
			name: "评论数",
			data: [21, 42, 65, 38, 24, 42, 71, 65, 38, 24]
		}],
		xaxis: {
			type: "text",
			
			categories: ["爱情片", "剧情片", "剧情片", "剧情片","爱情片", "剧情片", "剧情片", "剧情片","剧情片",
				"剧情片"
			]
	...
	(chart = new ApexCharts(document.querySelector("#top-category-area"), options)).render()
}
// 自定义部分：top10的类别 结束
...
// 各个区域的top10电影显示 开始
if (jQuery("#top-movie-area").length) {
	options = {
		chart: {
			height: 350,
			type: "bar"
		...
		series: [{
			name:"点击数:",
			data: [156464, 146464, 136464, 131464, 126464, 116464, 111464,111264, 106464, 101464,]
		}],
		xaxis: {
			categories: ["海贼王", "昨日青空", "火影忍者", "灵笼", "秦时明月", "亮剑", "雪鹰领主","四月是你的谎言","斗罗大陆","斗破苍穹"]
		}
	};
	(chart = new ApexCharts(document.querySelector("#top-movie-area"), options)).render()
}
...
//session统计之各访问时长、步长的占比 开始

if (jQuery("#time-ratio").length) {
	options = {
		chart: {
			width: 380,
			type: "pie"
		},
		labels: ["访问时长 1s-3s", "访问时长3s-7s", "访问时长7s-10s", "访问时长10s-30s", "访问时长30s+"],
		series: [10, 20, 20, 30, 20],
	...
	(chart = new ApexCharts(document.querySelector("#time-ratio"), options)).render()
}

if (jQuery("#step-ratio").length) {
	options = {
		chart: {
			width: 380,
			type: "pie"
		},
		labels: ["访问步长 1-3", "访问步长3-7", "访问步长7-10", "访问步长10-30", "访问步长30+"],
		series: [10, 20, 20, 30, 20],
		colors: ["#827af3", "#b47af3", "#6ce6f4", "#27b345", "#c8c8c8"],
		responsive: [{
			breakpoint: 480,
	...
	(chart = new ApexCharts(document.querySelector("#step-ratio"), options)).render()
}


//session统计之各访问时长、步长的占比 结束
...
if (jQuery("#am-simple-chart").length && am4core.ready(function() {
		...
		t.dataFields.category = "country", t.renderer.grid.template.location = 0, t.renderer.minGridDistance = 30, t.renderer
			.labels.template.adapter.add("dy", function(e, t) {
		...
		a.dataFields.valueY = "visits", a.dataFields.categoryX = "country", a.name = "Visits", a.columns.template.tooltipText =
			"{categoryX}: [bold]{valueY}[/]", a.columns.template.fillOpacity = .8;
		...
		e.yAxes.push(new am4charts.ValueAxis);
		var a = e.series.push(new am4charts.ColumnSeries);
		a.name = "Income", a.dataFields.valueY = "income", a.dataFields.categoryX = "year", a.columns.template.tooltipText =
			"[#fff font-size: 15px]{name} in {categoryX}:\n[/][#fff font-size: 20px]{valueY}[/] [#fff]{additional}[/]", a.columns
			.template.propertyFields.fillOpacity = "fillOpacity", a.columns.template.propertyFields.stroke = "stroke", a.columns
			.template.propertyFields.strokeWidth = "strokeWidth", a.columns.template.propertyFields.strokeDasharray =
			"columnDash", a.tooltip.label.textAlign = "middle";
		var n = e.series.push(new am4charts.LineSeries);
		n.name = "Expenses", n.dataFields.valueY = "expenses", n.dataFields.categoryX = "year", n.stroke = am4core.color(
			"#827af3"), n.strokeWidth = 3, n.propertyFields.strokeDasharray = "lineDash", n.tooltip.label.textAlign = "middle";
		var o = n.bullets.push(new am4charts.Bullet);
		o.fill = am4core.color("#fdd400"), o.tooltipText =
		...
		function n(t, a) {
			...
			n.name = a, n.dataFields.valueY = t, n.dataFields.categoryX = "year", n.sequencedInterpolation = !0, n.stacked = !0,
				n.columns.template.width = am4core.percent(60), n.columns.template.tooltipText =
				"[bold]{name}[/]\n[font-size:14px]{categoryX}: {valueY}";
			var o = n.bullets.push(new am4charts.LabelBullet);
			return o.label.text = "{valueY}", o.locationY = .5, n
		}
		a.renderer.inside = !0, a.renderer.labels.template.disabled = !0, a.min = 0, n("europe", "Europe"), n("namerica",
			"North America"), n("asia", "Asia-Pacific"), e.legend = new am4charts.Legend
		...
		t.dataFields.category = "year", t.renderer.inversed = !0, t.renderer.grid.template.location = 0, e.xAxes.push(new am4charts
			.ValueAxis).renderer.opposite = !0;
		var a = e.series.push(new am4charts.ColumnSeries);
		a.dataFields.categoryY = "year", a.dataFields.valueX = "income", a.name = "Income", a.columns.template.fillOpacity =
			.5, a.columns.template.strokeOpacity = 0, a.tooltipText = "Income in {categoryY}: {valueX.value}";
		var n = e.series.push(new am4charts.LineSeries);
		n.dataFields.categoryY = "year", n.dataFields.valueX = "expenses", n.name = "Expenses", n.strokeWidth = 3, n.tooltipText =
			"Expenses in {categoryY}: {valueX.value}";
		var o = n.bullets.push(new am4charts.CircleBullet);
		o.circle.fill = am4core.color("#fff"), o.circle.strokeWidth = 2, e.cursor = new am4charts.XYCursor, e.cursor.behavior =
			"zoomY", e.legend = new am4charts.Legend
		...
		a.dataFields.valueY = "value", a.dataFields.dateX = "date", a.tooltipText = "{value}", a.strokeWidth = 2, a.minBulletDistance =
			15, a.tooltip.background.cornerRadius = 20, a.tooltip.background.strokeOpacity = 0, a.tooltip.pointerOrientation =
			"vertical", a.tooltip.label.minWidth = 40, a.tooltip.label.minHeight = 40, a.tooltip.label.textAlign = "middle", a.tooltip
			.label.textValign = "middle";
		var n = a.bullets.push(new am4charts.CircleBullet);
		n.circle.strokeWidth = 2, n.circle.radius = 4, n.circle.fill = am4core.color("#fff"), n.states.create("hover").properties
			.scale = 1.3, e.cursor = new am4charts.XYCursor, e.cursor.behavior = "panXY", e.cursor.xAxis = t, e.cursor.snapToSeries =
			a, e.scrollbarY = new am4core.Scrollbar, e.scrollbarY.parent = e.leftAxesContainer, e.scrollbarY.toBack(), e.scrollbarX =
			new am4charts.XYChartScrollbar, e.scrollbarX.series.push(a), e.scrollbarX.parent = e.bottomAxesContainer, t.start =
			.79, t.keepSelection = !0
		...
		a.dataFields.valueY = "visits", a.dataFields.dateX = "date", a.strokeWidth = 2, a.minBulletDistance = 10, a.tooltipText =
			"{valueY}", a.tooltip.pointerOrientation = "vertical", a.tooltip.background.cornerRadius = 20, a.tooltip.background
			.fillOpacity = .5, a.tooltip.label.padding(12, 12, 12, 12), e.scrollbarX = new am4charts.XYChartScrollbar, e.scrollbarX
			.series.push(a), e.cursor = new am4charts.XYCursor, e.cursor.xAxis = t, e.cursor.snapToSeries = a
		...
		e.yAxes.push(new am4charts.ValueAxis);
		var a = e.series.push(new am4charts.LineSeries);
		a.dataFields.valueY = "value", a.dataFields.dateX = "date", a.strokeWidth = 3, a.fillOpacity = .5, e.scrollbarY =
			new am4core.Scrollbar, e.scrollbarY.marginLeft = 0, e.cursor = new am4charts.XYCursor, e.cursor.behavior = "zoomY",
		...
		o.category = "SE", o.endCategory = "SW", o.axisFill.fill = am4core.color("#fbc647"), o.axisFill.fillOpacity = .3, o.locations
			.endCategory = 0;
		var r = e.series.push(new am4charts.RadarSeries);
		r.dataFields.valueY = "value", r.dataFields.categoryX = "direction", r.name = "Wind direction", r.strokeWidth = 3, r
			.fillOpacity = .2
		...
		var t = e.yAxes.push(new am4charts.ValueAxis);
		t.renderer.labels.template.verticalCenter = "bottom", t.renderer.labels.template.horizontalCenter = "right", t.renderer
			.maxLabelPosition = .99, t.renderer.labels.template.paddingBottom = 1, t.renderer.labels.template.paddingRight = 3;
		var a = e.series.push(new am4charts.RadarSeries);
		a.bullets.push(new am4charts.CircleBullet), a.strokeOpacity = 0, a.dataFields.valueX = "x", a.dataFields.valueY =
			"y", a.name = "Series #1", a.sequencedInterpolation = !0, a.sequencedInterpolationDelay = 10, a.data = [{
				x: 83,
				y: 5.1
			}, {
				x: 44,
		...
		n.bullets.push(new am4charts.CircleBullet), n.strokeOpacity = 0, n.dataFields.valueX = "x", n.dataFields.valueY =
			"y", n.name = "Series #2", n.sequencedInterpolation = !0, n.sequencedInterpolationDelay = 10, n.data = [{
				x: 178,
				y: 1.3
			}, {
				x: 129,
		...
		o.bullets.push(new am4charts.CircleBullet), o.strokeOpacity = 0, o.dataFields.valueX = "x", o.dataFields.valueY =
			"y", o.name = "Series #3", o.sequencedInterpolation = !0, o.sequencedInterpolationDelay = 10, o.data = [{
				x: 419,
				y: 4.9
			}, {
				x: 417,
		...
		t.colors.list = [am4core.color("#827af3"), am4core.color("#fbc647"), am4core.color("#b47af3"), am4core.color(
				"#6ce6f4"), am4core.color("#27b345"), am4core.color("#c8c8c8")], t.dataFields.value = "litres", t.dataFields.category =
			"country"
		...
		var n = e.series.push(new am4charts.ColumnSeries);
		n.dataFields.valueY = "year2004", n.dataFields.categoryX = "country", n.clustered = !1, n.tooltipText =
			"GDP grow in {categoryX} (2004): [bold]{valueY}[/]";
		var o = e.series.push(new am4charts.ColumnSeries);
		o.dataFields.valueY = "year2005", o.dataFields.categoryX = "country", o.clustered = !1, o.columns.template.width =
			am4core.percent(50), o.tooltipText = "GDP grow in {categoryX} (2005): [bold]{valueY}[/]", e.cursor = new am4charts.XYCursor,
			e.cursor.lineX.disabled = !0, e.cursor.lineY.disabled = !0
...
if (jQuery("#high-basicline-chart").length && Highcharts.chart("high-basicline-chart", {
	...
	function showValues() {
		$("#alpha-value").html(chart.options.chart.options3d.alpha), $("#beta-value").html(chart.options.chart.options3d.beta),
...
if (jQuery("#high-gauges-chart").length && Highcharts.chart("high-gauges-chart", {
	...
	var categories = ["0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54",
	...
	Highcharts.chart("high-barwithnagative-chart", {
		...
		tooltip: {
			formatter: function() {
				return "<b>" + this.series.name + ", age " + this.point.category + "</b><br/>Population: " + Highcharts.numberFormat(
...
jQuery("#home-chart-04").length && am4core.ready(function() {
	...
	t.geodata = am4geodata_worldLow, t.projection = new am4maps.projections.Mercator, t.zoomControl = new am4maps.ZoomControl,
		t.homeZoomLevel = 2.5, t.homeGeoPoint = {
			latitude: 51,
			longitude: -23
		};
	...
	i.path = e, i.horizontalCenter = "middle", i.verticalCenter = "middle", i.scale = .7, i.fill = a.getFor(
		"alternativeBackground"), r.propertyFields.latitude = "latitude", r.propertyFields.longitude = "longitude", o.data = [{
		id: "london",
		svgPath: e,
		title: "London",
		latitude: 51.5002,
	...
	s.nonScalingStroke = !0, s.arrow.nonScaling = !0, s.arrow.width = 4, s.arrow.height = 6, s.stroke = a.getFor(
		"alternativeBackground"), s.fill = a.getFor("alternativeBackground"), s.line.strokeOpacity = .4, c.data = [{
		multiGeoLine: [
			[{
				latitude: 51.5002,
				longitude: -.1262
	...
	for (var t = [], a = 120, n = ["ra", "De", "Ca", "Ja", "Ri", "An"], o = 0; o < n.length; o++) a += Math.round((Math.random() <
		.5 ? 1 : -1) * Math.random() * 5), t.push({
		category: n[o],
		value: a
	});
	e.data = t;
	var r = e.xAxes.push(new am4charts.CategoryAxis);
	r.renderer.grid.template.location = 0, r.dataFields.category = "category", r.renderer.minGridDistance = 15, r.renderer
		.grid.template.location = .5, r.renderer.grid.template.strokeDasharray = "1,3", r.renderer.labels.template.rotation = -
		90, r.renderer.labels.template.horizontalCenter = "left", r.renderer.labels.template.location = .5, r.renderer.labels
		.template.adapter.add("dx", function(e, t) {
			return -t.maxRight / 2
		});
	...
	c.dataFields.categoryX = "category", c.dataFields.valueY = "value", c.tooltipText = "{valueY.value}", c.sequencedInterpolation = !
		0, c.fillOpacity = 0, c.strokeOpacity = 1, c.strokeDashArray = "1,3", c.columns.template.width = .01, c.tooltip.pointerOrientation =
		"horizontal";
	...
	for (var t = [], a = 100, n = 120, o = ["Rai", "Dem", "Caro", "Jac", "Rich", "Ano", "Amd", "Idal", "Joi", "Mar",
			"Cur", "Shl", "Meg"
		], r = 0; r < o.length; r++) n = (a += Math.round((Math.random() < .5 ? 1 : -1) * Math.random() * 5)) + Math.round(
		10 * Math.random()) + 3, t.push({
		category: o[r],
		open: a,
		close: n
	});
	...
	i.renderer.grid.template.location = 0, i.dataFields.category = "category", i.renderer.minGridDistance = 15, i.renderer
		.grid.template.location = .5, i.renderer.grid.template.strokeDasharray = "1,3", i.renderer.labels.template.rotation = -
		90, i.renderer.labels.template.horizontalCenter = "left", i.renderer.labels.template.location = .5, i.renderer.inside = !
		0, i.renderer.labels.template.adapter.add("dx", function(e, t) {
			return -t.maxRight / 2
		});
	...
	s.dataFields.categoryX = "category", s.dataFields.openValueY = "open", s.dataFields.valueY = "close", s.tooltipText =
		"open: {openValueY.value} close: {valueY.value}", s.sequencedInterpolation = !0, s.fillOpacity = 0, s.strokeOpacity =
		1, s.columns.template.width = .01, s.tooltip.pointerOrientation = "horizontal", s.bullets.create(am4charts.CircleBullet)
		.locationY = 1;
	...
	t.dataFields.value = "litres", t.dataFields.category = "country", t.innerRadius = am4core.percent(50), t.ticks.template
		.disabled = !0, t.labels.template.disabled = !0;
	var a = new am4core.RadialGradientModifier;
	a.brightnesses.push(-.8, -.8, -.5, 0, -.5), t.slices.template.fillModifier = a, t.slices.template.strokeModifier = a,
		t.slices.template.strokeOpacity = .4, t.slices.template.strokeWidth = 0
});
var animationData = {
		v: "5.5.2",
		fr: 60,
...
$(".table-add").on("click", "i", () => {
	...
	i(o), $(".next").click(function() {
		e = $(this).parent(), t = $(this).parent().next(), $("#top-tab-list li").eq($("fieldset").index(t)).addClass(
			"active"), $("#top-tab-list li").eq($("fieldset").index(e)).addClass("done"), t.show(), e.animate({
			opacity: 0
		}, {
			step: function(a) {
				n = 1 - a, e.css({
		...
		e = $(this).parent(), a = $(this).parent().prev(), $("#top-tab-list li").eq($("fieldset").index(e)).removeClass(
			"active"), $("#top-tab-list li").eq($("fieldset").index(a)).removeClass("done"), a.show(), e.animate({
			opacity: 0
		}, {
			step: function(t) {
				n = 1 - t, e.css({
	...
	t.hide(), e.click(function(a) {
		...
		var n = $($(this).attr("href")),
			o = $(this);
		o.hasClass("disabled") || (e.addClass("active"), o.parent().addClass("active"), t.hide(), n.show(), n.find(
			"input:eq(0)").focus())
	}), a.click(function() {
		...
		for (var r = 0; r < n.length; r++) n[r].validity.valid || (o = !1, $(n[r]).closest(".form-group").addClass(
			"has-error"));
	...
	i(o), $(".next").click(function() {
		e = $(this).parent(), t = $(this).parent().next(), $("#top-tabbar-vertical li").eq($("fieldset").index(t)).addClass(
			"active"), t.show(), e.animate({
			opacity: 0
		}, {
			step: function(a) {
				n = 1 - a, e.css({
		...
		e = $(this).parent(), a = $(this).parent().prev(), $("#top-tabbar-vertical li").eq($("fieldset").index(e)).removeClass(
			"active"), a.show(), e.animate({
			opacity: 0
		}, {
			step: function(t) {
				n = 1 - t, e.css({
	...
	$(".progress-round").each(function() {
		...
		t > 0 && (t <= 50 ? n.css("transform", "rotate(" + e(t) + "deg)") : (n.css("transform", "rotate(180deg)"), a.css(
			"transform", "rotate(" + e(t - 50) + "deg)")))
	})
});
